...
GROUND_TRUTH = [
    ("users-patch-idor", "high", "users.js", range(80, 98),
     ["never verifies", "req.user.id === req.params", "any authenticated user can", "IDOR", "object-level"]),
    ("users-patch-email", "med", "users.js", range(90, 93),
     ["email", "uniqueness"]),
    ("users-login-no-ratelimit", "med", "users.js", range(43, 59),
     ["login", "rate limit", "brute"]),
    ("users-password-unvalidated", "med", "users.js", range(12, 41),
     ["password", "never validate", "no password"]),
    ("reviews-flag-self-hide", "high", "reviews.js", range(64, 80),
     ["flag", "3 times", "3×", "auto-hide", "flags.length", "moderation bypass"]),
    ("reviews-flag-unvalidated", "med", "reviews.js", range(70, 73),
     ["flag", "REVIEW_FLAGS", "never validated"]),
    ("reviews-moderate-unhide", "low", "reviews.js", range(88, 92),
     ["hidden", "unhide", "=== true"]),
    ("products-delete-ownership", "high", "products.js", range(98, 110),
     ["DELETE", "sellerId", "ownership", "another seller"]),
    ("products-restock-ownership", "high", "products.js", range(112, 126),
     ["restock", "ownership", "another seller", "any seller"]),
    ("products-restock-delta", "high", "products.js", range(118, 122),
     ["delta", "concatenation", "NaN", "negative"]),
    ("products-patch-novalidate", "high", "products.js", range(85, 92),
     ["PATCH", "re-validat", "validateProduct", "negative"]),
    ("orders-all-idor", "high", "orders.js", range(48, 53),
     ["all=1", "?all", "every order", "admin sees all", "enumerate"]),
    ("orders-refund-amount", "med", "orders.js", range(95, 99),
     ["amountCents", "refund", "no validation", "negative"]),
    ("orders-refund-status", "med", "orders.js", range(96, 99),
     ["refund", "status", "precondition", "delivered"]),
    ("orders-cancel-status", "med", "orders.js", range(79, 83),
     ["cancel", "status", "shipped", "delivered"]),
    ("orders-expired-coupon", "med", "orders.js", range(22, 30),
     ["coupon", "expire", "isExpired"]),
]
...
def extract_report(run_dir: Path) -> str:
    ...
    root = db.execute("SELECT id FROM session WHERE parent_id IS NULL ORDER BY time_created DESC LIMIT 1").fetchone()
    if not root:
        return ""
    texts = []
    for r in db.execute("SELECT data FROM part WHERE session_id=? ORDER BY time_created", (root["id"],)):
        d = json.loads(r["data"])
...
def line_numbers(report: str) -> set[int]:
    return {int(n) for n in re.findall(r"\.js:(\d+)", report)}
...
def lines_per_handler(report: str) -> dict[str, set[int]]:
    ...
    for handler, num in re.findall(r"(\w+\.js):(\d+)", report):
        out.setdefault(handler, set()).add(int(num))
    return out
...
def score(report: str) -> dict:
    ...
    findings = len(re.findall(r"\*\*(high|medium|med|low)\*\*|—\s*(high|medium|low)\b", report, re.I))
...
def main() -> None:
    ...
    verdict = "PASS" if len(sb["ground_truth_found"]) >= len(sa["ground_truth_found"]) - 1 and len(sb["handlers_covered"]) == 4 else "REVIEW"
