...
from rrc.orchestrator_contract import OrchestratorTask, PlanSpecPacket, Profile
...
class OrchestratorPolicy:
    ...
    def decide(self, task: OrchestratorTask) -> PolicyDecision:
        ...
        score = _slot_weight(slot_count) + _oracle_weight(oracle_count) + _shape_weight(shape_words)
    ...
    def packet_token_count(self, task: OrchestratorTask, packet: PlanSpecPacket) -> int:
    ...
    def validate_packet(
        ...
        if decision.profile == "lean":
            ...
            if not 2 <= len(steps) <= 4 or not 2 <= len(packet.acceptance) <= 4:
                raise PacketValidationError("detailed packets require 2-4 plan steps and tests")
...
def _case_shape(task: OrchestratorTask) -> str:
    return task.case_shape if task.case_shape is not None else task.text


def _slot_values(task: OrchestratorTask) -> Mapping[str, object]:
    return task.slot_values if task.slot_values is not None else task.params


def _slot_names(task: OrchestratorTask) -> tuple[str, ...]:
...
def _packet_texts(task: OrchestratorTask, packet: PlanSpecPacket) -> tuple[str, ...]:
...
def packet_token_count(task: OrchestratorTask, packet: PlanSpecPacket) -> int:
