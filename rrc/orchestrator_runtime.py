...
from rrc.contract import Complete, PlanSpecPacket, PlanSpecTemplate, Profile, Task
...
def _required_case_shape(task: Task) -> str:
...
def _render_value(
    ...
    if isinstance(value, dict):
        return {
            key: _render_value(item, slot_names, slot_values) for key, item in value.items()
        }
    return value


def _worker_prompt(case_shape: str, task: Task, template: PlanSpecTemplate) -> str:
